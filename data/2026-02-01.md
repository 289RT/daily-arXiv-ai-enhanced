...
TL;DR: 该论文通过分子动力学模拟研究了AKT在PIP₃膜上的结合模式，揭示了PH结构域和激酶结构域如何协同调控AKT的膜结合构象。


<details>
  <summary>Details</summary>
Motivation: AKT的激活与膜结合相关，但其PH结构域和激酶结构域如何合作调控这一过程尚不清楚。

Method: 采用分子动力学模拟，研究了全长AKT在含PIP₃脂质双层上的结合模式。

Result: 发现四种不同的膜结合模式，其中PIP₃不仅与PH结构域结合，还与激酶结构域的基本残基相互作用。膜上PIP₃浓度影响AKT的偏好取向。

Conclusion: PH结构域和激酶结构域的脂质识别协同塑造了AKT的膜结合构象，为AKT激活机制提供了新见解。
